...
export const IndexSubscriptionDistributions: FC<{
    ...
    const [indexUpdatedEventPaging, setIndexUpdatedEventPaging] = useState<SkipPaging>(
        createSkipPaging({
            take: 10,
        })
    );
    ...
    const subscriptionUnitsUpdatedEventsQuery = sfSubgraph.useSubscriptionUnitsUpdatedEventsQuery({
        chainId: network.chainId,
        filter: {
    ...
    const subscriptionUnitsUpdatedEvents: SubscriptionUnitsUpdatedEvent[] |
        undefined = useMemo(
    ...
    const indexUpdatedEvents: IndexUpdatedEvent[] | undefined = indexUpdatedEventsQuery.data?.data ?? [];
        ...
        () => [
            ...
            },
            {
                ...
                renderCell: (params) => {
                    ...
                    let closestSubscriptionUnitsUpdatedEvent = subscriptionUnitsUpdatedEvents.find(
                        (x) => x.timestamp <= indexUpdatedEvent.timestamp
                    )!;

                    if (indexUpdatedEvent.timestamp ===
                        ...
                        if (subscriptionUnitsUpdatedEventLogIndex > indexUpdatedEventLogIndex) {
                    ...
                    const subscriptionDistributionAmount = indexDistributionAmount.mul(subscriptionUnits);
