...
export const PoolMemberDistributions: FC<{
    ...
    const [
        instantDistributionUpdatedEventPaging, setInstantDistributionUpdatedEventPaging,
    ] = useState<SkipPaging>(
        createSkipPaging({
            take: 10,
        })
    ...
    const [
        instantDistributionUpdatedEventOrdering, setInstantDistributionUpdatedEventOrdering,
    ] = useState<Ordering<InstantDistributionUpdatedEvent_OrderBy> | undefined>({
        orderBy: "timestamp",
        orderDirection: "desc",
    });

    const memberUnitsUpdatedEventsQuery = sfGdaSubgraph.usePoolMemberUnitsUpdatedEventsQuery({
        chainId: network.chainId,
        filter: {
    ...
    const subscribersUnitsUpdatedEvents: PoolMemberUnitsUpdatedEvent[] |
        undefined = useMemo(
            () => memberUnitsUpdatedEventsQuery.data?.items ?? [],
            [memberUnitsUpdatedEventsQuery.data]
        );

    const instantDistributionUpdatedEventsQuery = sfGdaSubgraph.useInstantDistributionUpdatedEventsQuery(
        ...
        () => [
            ...
            },
            {
                ...
                renderCell: (params) => {
                    ...
                    const instantDistributionUpdatedEvent = params.row as InstantDistributionUpdatedEvent;

                    let closestMembershipUnitsUpdatedEvent = subscribersUnitsUpdatedEvents.find(
                        (x) => x.timestamp <= instantDistributionUpdatedEvent.timestamp
                    )!;

                    if (instantDistributionUpdatedEvent.timestamp ===
                        ...
                        if (subscribersUnitsUpdatedEventLogIndex >
                    ...
                    const subscribersDistributionAmount = BigNumber.from(instantDistributionUpdatedEvent.actualAmount).mul(subscribersUnits);
