...
export const getTotalAmountReceivedFromPoolMember = (member: PoolMemberInput, pool: PoolInput): {
  ...
  const poolCurrentTotalAmountDistributedDelta = BigNumber.from(pool.flowRate).mul(
    currentTimestamp - pool.updatedAtTimestamp
  );
  ...
  const memberCurrentTotalAmountReceivedDelta = poolCurrentTotalAmountDistributed
  ...
  const memberFlowRate = BigNumber.from(pool.flowRate).mul(memberUnits).div(poolUnits);
...
export const TotalAmountReceivedFromPoolMember: FC<{
  member: PoolMemberInput,
  pool: PoolInput,
  children: (output: ReturnType<typeof getTotalAmountReceivedFromPoolMember>) => PropsWithChildren["children"]
