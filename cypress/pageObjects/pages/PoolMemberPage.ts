...
const UNITS_TABLE_DATE =   "[data-cy=units-updated-grid] .MuiDataGrid-cell[data-field=timestamp]";
const UNITS_TABLE_UNITS =   "[data-cy=units-updated-grid] .MuiDataGrid-cell[data-field=units]";
...
export class PoolMemberPage extends BasePage {
    static validatePoolMemberGeneralData(network: string) {
        cy.fixture("gdaData").then((data) => {
          ...
          this.containsText(POOL_TOKEN, data[network].poolMembers[0].pool.token.symbol);
          this.containsText(POOL_ADMIN, ethers.utils.getAddress(data[network].poolMembers[0].pool.admin.id));
          this.containsText(POOL_ADDRESS, data[network].poolMembers[0].pool.id);
          this.containsText(POOL_MEMBER, ethers.utils.getAddress(data[network].poolMembers[0].id.split("-")[2]));
          this.containsText(POOL_UNITS, `${data[network].poolMembers[0].units} / ${data[network].poolMembers[0].pool.totalUnits} (${calculatePoolPercentage(
            ...
            const createdAtDate = new Date(
              ...
              this.containsText(POOL_APPROVAL_STATUS,data[network].poolMembers[0].isConnected ? "Yes" : "No")
              //FIX the data shown in console is wrong atm
              this.containsText(POOL_TOTAL_AMOUNT_CLAIMED,data[network].poolMembers[0].totalAmountClaimed / 1e18)
        });
      }
      ...
      static validatePoolMemberUnitUpdateTable(network: string) {
        cy.fixture("gdaData").then((data) => {
          data[network].poolMemberUnitUpdateTable.forEach((entry: any, index: number) => {
            cy.get(UNITS_TABLE_DATE)
              .eq(index)
              .should("have.text", entry.date);
